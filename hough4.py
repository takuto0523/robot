...
def pick_up_colored_balls():
    ...
    cap.set(cv2.CAP_PROP_FPS, 30)  # カメラのFPSを30に設定
    cap.set(3, 640)
    cap.set(4, 480)
    ...
    while True:
        ...
        lower_yellow = np.array([10, 70, 170])
        upper_yellow = np.array([42, 200, 255])
        ...
        for img_color, mask, color_name in [(img_blue, mask_blue, 'Blue'), (img_red, mask_red, 'Red'), (img_yellow, mask_yellow, 'Yellow')]:
            ...
            circles = cv2.HoughCircles(img_color, cv2.HOUGH_GRADIENT, 1, 20, param1=50, param2=20, minRadius=5, maxRadius=150)
        ...
        k = cv2.waitKey(33)  # 1秒を1000ミリ秒、30fpsの場合は1000ミリ秒 / 30フレーム ≒ 33ミリ秒
        if k == 27:
            break
